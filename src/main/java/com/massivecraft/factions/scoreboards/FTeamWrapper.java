...
public class FTeamWrapper {
	...
	public static void applyUpdatesLater(final Faction faction) {
		...
		if (!SavageFactionsPlugin.plugin.getConfig().getBoolean("scoreboard.default-prefixes", false) || SavageFactionsPlugin.plugin.getConfig().getBoolean("See-Invisible-Faction-Members")) {
			return;
		}


		if (updating.add(faction)) {
			Bukkit.getScheduler().runTask(SavageFactionsPlugin.plugin, new Runnable() {
				@Override
				public void run() {
	...
	public static void applyUpdates(Faction faction) {
		...
		if (wrapper != null && Factions.getInstance().getFactionById(faction.getUniqueId()) == null) {
	...
	private void updatePrefixes() {
		if (SavageFactionsPlugin.plugin.getConfig().getBoolean("scoreboard.default-prefixes", false)) {
	...
	private void updatePrefix(FScoreboard fboard) {
		if (SavageFactionsPlugin.plugin.getConfig().getBoolean("scoreboard.default-prefixes", false)) {
			...
			if (SavageFactionsPlugin.plugin.getConfig().getBoolean("See-Invisible-Faction-Members", false)) {
				team.setCanSeeFriendlyInvisibles(true);
			}

			if ((SavageFactionsPlugin.plugin.getConfig().getBoolean("ffocus.Enabled")) && (fplayer.getFaction() != null) && (fplayer.getFaction().getFocused() != null)) {
				for (FPlayer fp : faction.getFPlayersWhereOnline(true)) {
					if (fplayer.getFaction().getFocused().equalsIgnoreCase(fp.getName())) {
						team.setPrefix(ChatColor.translateAlternateColorCodes('&', SavageFactionsPlugin.plugin.getConfig().getString("ffocus.Prefix", "&7»&b")));
						focused = true;
					}
				}
			}
