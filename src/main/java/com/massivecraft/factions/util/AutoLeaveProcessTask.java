	...
	public void run() {
		...
		while (iterator.hasNext()) {
			...
			if (!fplayer.willAutoLeave()) {
				SavageFactionsPlugin.plugin.debug(Level.INFO, fplayer.getName() + " was going to be auto-removed but was set not to.");
				continue;
			}

			if (fplayer.isOffline() && now - fplayer.getLastLoginTime() > toleranceMillis) {
				if (Conf.logFactionLeave || Conf.logFactionKick) {
					SavageFactionsPlugin.plugin.log("Player " + fplayer.getName() + " was auto-removed due to inactivity.");
				}
