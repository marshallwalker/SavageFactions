...
public class MiscUtil {
	...
	public static ArrayList<String> validateTag(String str) {
		...
		if (getComparisonString(str).length() < Conf.factionTagLengthMin) {
			errors.add(SavageFactionsPlugin.plugin.txt.parse(TL.GENERIC_FACTIONTAG_TOOSHORT.toString(), Conf.factionTagLengthMin));
		}

		if (str.length() > Conf.factionTagLengthMax) {
			errors.add(SavageFactionsPlugin.plugin.txt.parse(TL.GENERIC_FACTIONTAG_TOOLONG.toString(), Conf.factionTagLengthMax));
		}

		for (char c : str.toCharArray()) {
			if (!substanceChars.contains(String.valueOf(c))) {
				errors.add(SavageFactionsPlugin.plugin.txt.parse(TL.GENERIC_FACTIONTAG_ALPHANUMERIC.toString(), c));
			}
		}
	...
	public static Iterable<FPlayer> rankOrder(Iterable<FPlayer> players) {
		...
		for (FPlayer player : players) {
			...
			if (player.getRole() == null) {
				...
				SavageFactionsPlugin.plugin.log(Level.WARNING, String.format("Player %s had null role. Setting them to normal. This isn't good D:", player.getName()));
			}
