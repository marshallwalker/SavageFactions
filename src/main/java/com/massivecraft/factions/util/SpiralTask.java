...
public abstract class SpiralTask implements Runnable {
	...
	public SpiralTask(FLocation fLocation, int radius) {
		...
		if (this.world == null) {
			SavageFactionsPlugin.plugin.log(Level.WARNING, "[SpiralTask] A valid world must be specified!");
			this.stop();
			return;
		}
		...
		this.setTaskID(Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(SavageFactionsPlugin.plugin, this, 2, 2));
	}
	...
	public void finish() {
//		SavageFactionsPlugin.plugin.log("SpiralTask successfully completed!");
		this.stop();
	}
