...
public class Persist {
    ...
    public boolean save(Object instance, File file) {
        try {
            SavageFactionsPlugin.plugin.objectMapper.writeValue(file, instance);
            return true;
//			return DiscUtil.writeCatch(file, SavageFactionsPlugin.plugin.objectMapper.writeValueAsString(instance), true);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//		}
        ...
        try {
            ObjectMapper objectMapper = SavageFactionsPlugin.plugin.objectMapper;
    ...
    public <T> T load(TypeReference type, String name) {
        return (T) load(type, getFile(name));
    }

    @SuppressWarnings("unchecked")
    public <T> T load(TypeReference type, File file) {
//		String content = DiscUtil.readCatch(file);
//		if (content == null) {
//			return null;
//		}
        ...
        try {
            return (T) SavageFactionsPlugin.plugin.objectMapper.readValue(file, type);
        } catch (Exception ex) {    // output the error message rather than full stack trace; error parsing the file, most likely
            p.log(Level.WARNING, ex.getMessage());
        }
