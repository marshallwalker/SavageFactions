...
public class TagUtil {
	...
	public static String parsePlaceholders(Player player, String line) {
		if (SavageFactionsPlugin.plugin.isClipPlaceholderAPIHooked() && player.isOnline()) {
			line = PlaceholderAPI.setPlaceholders(player, line);
		}

		if (SavageFactionsPlugin.plugin.isMVdWPlaceholderAPIHooked() && player.isOnline()) {
	...
	protected static List<FancyMessage> getFancy(Faction target, FPlayer fme, TagReplacer type, String prefix) {
		...
		switch (type) {
			case ALLIES_LIST:
				FancyMessage currentAllies = SavageFactionsPlugin.plugin.txt.parseFancy(prefix);
			...
			case ENEMIES_LIST:
				FancyMessage currentEnemies = SavageFactionsPlugin.plugin.txt.parseFancy(prefix);
			...
			case TRUCES_LIST:
				FancyMessage currentTruces = SavageFactionsPlugin.plugin.txt.parseFancy(prefix);
			...
			case ONLINE_LIST:
				FancyMessage currentOnline = SavageFactionsPlugin.plugin.txt.parseFancy(prefix);
			...
			case OFFLINE_LIST:
				FancyMessage currentOffline = SavageFactionsPlugin.plugin.txt.parseFancy(prefix);
	...
	private static List<String> tipFaction(Faction faction) {
		...
		for (String line : SavageFactionsPlugin.plugin.getConfig().getStringList("tooltips.list")) {
	...
	private static List<String> tipPlayer(FPlayer fplayer) {
		List<String> lines = new ArrayList<>();
		for (String line : SavageFactionsPlugin.plugin.getConfig().getStringList("tooltips.show")) {
