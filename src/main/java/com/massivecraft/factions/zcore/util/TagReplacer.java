...
public enum TagReplacer {
    ...
    protected String getValue() {
        switch (this) {
            ...
            case MAX_ALLIES:
                if (SavageFactionsPlugin.plugin.getConfig().getBoolean("max-relations.enabled", true)) {
                    return String.valueOf(SavageFactionsPlugin.plugin.getConfig().getInt("max-relations.ally", 10));
                }
                return TL.GENERIC_INFINITY.toString();
            case MAX_ENEMIES:
                if (SavageFactionsPlugin.plugin.getConfig().getBoolean("max-relations.enabled", true)) {
                    return String.valueOf(SavageFactionsPlugin.plugin.getConfig().getInt("max-relations.enemy", 10));
                }
                return TL.GENERIC_INFINITY.toString();
            case MAX_TRUCES:
                if (SavageFactionsPlugin.plugin.getConfig().getBoolean("max-relations.enabled", true)) {
                    return String.valueOf(SavageFactionsPlugin.plugin.getConfig().getInt("max-relations.truce", 10));
                }
                return TL.GENERIC_INFINITY.toString();
            case MAX_WARPS:
                return String.valueOf(SavageFactionsPlugin.plugin.getConfig().getInt("max-warps", 5));
            default:
        }
        return null;
    }
    ...
    protected String getValue(Faction fac, FPlayer fp) {
        ...
        boolean minimal = SavageFactionsPlugin.plugin.getConfig().getBoolean("minimal-show", false);

        if (fp != null) {
            switch (this) {
                case HEADER:
                    return SavageFactionsPlugin.plugin.txt.titleize(fac.getTag(fp));
                case PLAYER_NAME:
                    return fp.getName();
                case FACTION:
                ...
                case PLAYER_GROUP:
                    return SavageFactionsPlugin.plugin.getPrimaryGroup(Bukkit.getOfflinePlayer(fp.getId()));
                case PLAYER_BALANCE:
                    return Econ.isSetup() ? fp.getBalanceFormatted() : TL.ECON_OFF.format("balance");
                case PLAYER_POWER:
        ...
        switch (this) {
            ...
            case PEACEFUL:
                return fac.isPeaceful() ? relationColor.neutral + TL.COMMAND_SHOW_PEACEFUL.toString() : "";
            case PERMANENT:
            ...
            case RAIDABLE:
                boolean raid = SavageFactionsPlugin.plugin.getConfig().getBoolean("hcf.raidable", false) && fac.getLandRounded() >= fac.getPowerRounded();
            ...
            case BANK_BALANCE:
                if (Econ.shouldBeUsed()) {
                    return configuration.faction.bank.enabled ? fac.getBalanceFormatted() : minimal ? null : TL.ECON_OFF.format("balance");
                }
