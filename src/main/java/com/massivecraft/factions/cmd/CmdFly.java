...
public class CmdFly extends FCommand {
	...
	public static void startParticles() {
		...
		id = Bukkit.getScheduler().scheduleSyncRepeatingTask(SavageFactionsPlugin.plugin, new Runnable() {
			@Override
			public void run() {
				for (String name : flyMap.keySet()) {
					...
					if (FPlayers.getInstance().getByPlayer(player).isVanished()) {
						...
						if (SavageFactionsPlugin.plugin.getConfig().getBoolean("ffly.Particles.Enable-While-Vanished")) {
							return;
						}
						continue;
					}
					if (SavageFactionsPlugin.plugin.useNonPacketParticles) {
	...
	public static void startFlyCheck() {
		flyid = Bukkit.getScheduler().scheduleSyncRepeatingTask(SavageFactionsPlugin.plugin, new Runnable() {
			@Override
			public void run() throws ConcurrentModificationException { //threw the exception for now, until I recode fly :( Cringe.
				...
				if (flyMap.keySet().size() != 0) {
					for (String name : flyMap.keySet()) {
						...
						if (SavageFactionsPlugin.plugin.serverVersion != ServerVersion.MC_V17 && player.getGameMode() == GameMode.SPECTATOR) {
							continue;
						}
