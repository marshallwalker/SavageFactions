...
public class CmdTntFill extends FCommand {
	...
	@Override
	public void perform() {
		if (!SavageFactionsPlugin.plugin.getConfig().getBoolean("Tntfill.enabled")) {
		...
		if (radius > SavageFactionsPlugin.plugin.getConfig().getInt("Tntfill.max-radius")) {
			fme.msg(TL.COMMAND_TNTFILL_RADIUSMAX.toString().replace("{max}", SavageFactionsPlugin.plugin.getConfig().getInt("Tntfill.max-radius") + ""));
			return;
		}
		if (amount > SavageFactionsPlugin.plugin.getConfig().getInt("Tntfill.max-amount")) {
			fme.msg(TL.COMMAND_TNTFILL_AMOUNTMAX.toString().replace("{max}", SavageFactionsPlugin.plugin.getConfig().getInt("Tntfill.max-amount") + ""));
			return;
		}
	...
	public void takeTnt(int amount) {
		...
		if (fme.getFaction().getTnt() + amount > SavageFactionsPlugin.plugin.getConfig().getInt("ftnt.Bank-Limit")) {
