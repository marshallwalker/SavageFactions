...
public abstract class FRelationCommand extends FCommand {
	...
	private boolean hasMaxRelations(Faction them, Relation targetRelation) {
		int max = SavageFactionsPlugin.plugin.getConfig().getInt("max-relations." + targetRelation.toString(), -1);
		if (SavageFactionsPlugin.plugin.getConfig().getBoolean("max-relations.enabled", false)) {
			if (max != -1) {
