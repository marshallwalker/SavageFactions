	...
	public void perform() {
		...
		if (!success.isEmpty()) {
			if (sender instanceof Player) {
				sendMessage(success);
				SavageFactionsPlugin.plugin.log(success + TL.COMMAND_CONFIG_LOG.format((Player) sender));
			} else  // using SavageFactionsPlugin.plugin.log() instead of sendMessage if run from server console so that "[Factions v#.#.#]" is prepended in server log
			{
				SavageFactionsPlugin.plugin.log(success);
			}
		}
		// save change to disk
		Conf.save();
