...
public class CmdStuck extends FCommand {
	...
	public void perform() {
		...
		final long delay = SavageFactionsPlugin.plugin.getConfig().getLong("hcf.stuck.delay", 30);
		final int radius = SavageFactionsPlugin.plugin.getConfig().getInt("hcf.stuck.radius", 10);

		if (SavageFactionsPlugin.plugin.getStuckMap().containsKey(player.getUniqueId())) {
			long wait = SavageFactionsPlugin.plugin.getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
			...
			final int id = Bukkit.getScheduler().runTaskLater(SavageFactionsPlugin.plugin, new BukkitRunnable() {
				...
				public void run() {
					...
					if (world.getUID() != player.getWorld().getUID() || sentAt.distance(player.getLocation()) > radius) {
						...
						SavageFactionsPlugin.plugin.getTimers().remove(player.getUniqueId());
						SavageFactionsPlugin.plugin.getStuckMap().remove(player.getUniqueId());
						return;
					}
						...
						public boolean work() {
							...
							if (faction.isWilderness() && !chunk.isOutsideWorldBorder(buffer)) {
								...
								if (!Essentials.handleTeleport(player, tp)) {
									player.teleport(tp);
									SavageFactionsPlugin.plugin.debug("/f stuck used regular teleport, not essentials!");
								}
								this.stop();
								return false;
							}
			...
			SavageFactionsPlugin.plugin.getTimers().put(player.getUniqueId(), System.currentTimeMillis() + (delay * 1000));
			long wait = SavageFactionsPlugin.plugin.getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
			String time = DurationFormatUtils.formatDuration(wait, TL.COMMAND_STUCK_TIMEFORMAT.toString(), true);
			msg(TL.COMMAND_STUCK_START, time);
			SavageFactionsPlugin.plugin.getStuckMap().put(player.getUniqueId(), id);
		}
	}
