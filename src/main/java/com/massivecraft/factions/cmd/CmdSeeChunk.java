...
public class CmdSeeChunk extends FCommand {
	...
	public CmdSeeChunk() {
		...
		interval = SavageFactionsPlugin.plugin.getConfig().getLong("see-chunk.interval", 10L);
		if (effect == null) {
	...
	private void startTask() {
		taskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(SavageFactionsPlugin.plugin, new Runnable() {
			@Override
			public void run() {
	...
	private void showPillar(Player player, World world, int blockX, int blockZ) {
		for (int blockY = 0; blockY < player.getLocation().getBlockY() + 30; blockY++) {
			...
			if (useParticles) {
				...
				Material type = blockY % 5 == 0 ? SavageFactionsPlugin.plugin.REDSTONE_LAMP_ON : SavageFactionsPlugin.plugin.STAINED_GLASS;
