...
public class CmdTnt extends FCommand {
    ...
    public CmdTnt() {
        ...
        optionalArgs.put("add/take", "");
        optionalArgs.put("amount", "number");
    ...
    @Override
    public void perform() {
        if (!SavageFactionsPlugin.plugin.getConfig().getBoolean("ftnt.Enabled")) {
        ...
        if (args.size() == 2) {
            if (args.get(0).equalsIgnoreCase("add") || args.get(0).equalsIgnoreCase("a")) {
                ...
                fme.sendMessage(SavageFactionsPlugin.plugin.color(TL.COMMAND_TNT_AMOUNT.toString().replace("{amount}", fme.getFaction().getTnt() + "")));
                return;
