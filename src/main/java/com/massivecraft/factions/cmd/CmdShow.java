...
public class CmdShow extends FCommand {
    ...
    @Override
    public void perform() {
        Faction faction = argIsSet(0) ? argAsFaction(0) : myFaction;

        if (faction == null)
            return;

        if (fme != null && !fme.getPlayer().hasPermission("factions.show.bypassexempt")
                && SavageFactionsPlugin.plugin.getConfig().getStringList("show-exempt").contains(faction.getTag())) {
        ...
        List<String> show = SavageFactionsPlugin.plugin.getConfig().getStringList("show");
