...
public class CmdHelp extends FCommand {
	...
	@Override
	public void perform() {
		if (SavageFactionsPlugin.plugin.getConfig().getBoolean("use-old-help", true)) {
			if (helpPages == null) {
				updateHelp();
			}
		...
		ConfigurationSection help = SavageFactionsPlugin.plugin.getConfig().getConfigurationSection("help");
		if (help == null) {
			help = SavageFactionsPlugin.plugin.getConfig().createSection("help"); // create new help section
		...
		for (String helpLine : page) {
			sendMessage(SavageFactionsPlugin.plugin.txt.parse(helpLine));
		}
	}
	...
	public void updateHelp() {
		...
		if (Econ.isSetup() && Conf.econEnabled && bankConfiguration.enabled) {
			pageLines = new ArrayList<>();
			pageLines.add("");
		...
		//TODO:TL
		pageLines.add(p.txt.parse("<i>Note: " + p.cmdBase.cmdUnclaim.getUseageTemplate(false) + SavageFactionsPlugin.plugin.txt.parse("<i>") + " works on safe/war zones as well."));
